...
(def ^:private subject-ns (create-ns 'zigar.lifecycle-subject))
(def ^:private the-var (intern subject-ns 'f))

(defn- spec0 []
  (spec/build-spec {:ns 'zigar.lifecycle-subject :name 'f :signature '[x :i64 :ret :i64]}))
...
(defn- define!
  ...
  (get-in (meta the-var) [:zigar/info :status]))
   ...
   {:define
    ...
    {:requires      (fn [s] (:defined? s))
     :command       (fn [] {:result         (define-fail!)
                            :failed-attempt? (some? (:zigar/failed-attempt (meta the-var)))
                            :last-good       (call! 7)})
    ...
    {:requires      (fn [s] (:defined? s))
     ...
     ;; invoker's :zigar/arity guards the unwrapped path.
...
(deftest clean-removes-a-cache-tree-without-disturbing-a-live-binding
  ...
  (let [scratch (str (java.nio.file.Files/createTempDirectory
                      "zigar-clean" (make-array java.nio.file.attribute.FileAttribute 0))
                     "/cache")]
