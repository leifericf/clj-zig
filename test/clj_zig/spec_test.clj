...
(deftest spec-for-type-maps-scalars
  ...
  (is (= 'double? (spec/spec-for-type {:kind :scalar :name :f64})))
  (is (= 'boolean? (spec/spec-for-type {:kind :scalar :name :bool})))
  (is (= 'nil? (spec/spec-for-type {:kind :scalar :name :void}))))

(deftest spec-for-type-maps-string
  (is (= 'string? (spec/spec-for-type {:kind :string}))))

(deftest spec-for-type-maps-bytes
  (is (= 'bytes? (spec/spec-for-type {:kind :bytes}))))

(deftest spec-for-type-maps-slice
  (is (= '(clojure.spec.alpha/coll-of int?)
         (spec/spec-for-type {:kind :slice :const? true
                              :of {:kind :scalar :name :u8}}))))

(deftest spec-for-type-maps-optional
  (is (= '(clojure.spec.alpha/nilable int?)
         (spec/spec-for-type {:kind :optional
...
(deftest spec-for-type-maps-handle
  (is (= 'some? (spec/spec-for-type {:kind :handle
                                     :of {:kind :named :name 'Box}}))))

(deftest spec-for-type-maps-named-struct
  (is (= 'map? (spec/spec-for-type {:kind :named :name 'Point
                                    :layout {:fields []}}))))
...
(deftest spec-for-type-maps-owned-slice
  (let [t {:kind :owned
           :of {:kind :slice
                :of {:kind :scalar :name :i64}}}]
    (is (= '(clojure.spec.alpha/coll-of int?)
           (spec/spec-for-type t)))))

(deftest spec-for-type-maps-error-union
  (is (= 'int?
         (spec/spec-for-type {:kind :error-union
                              :error 'MyError
                              :of {:kind :scalar :name :i64}}))))
