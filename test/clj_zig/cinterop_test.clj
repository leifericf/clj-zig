(ns clj-zig.cinterop-test
  ...
  links a C library through the descriptor's `:c/link`. This exercises
...
(deftest cimport-of-math-h-links-libm-and-round-trips
  (testing "a body that calls c.sqrt links libm via :c/link and returns 5.0"
    (let [dir  (scratch-dir)
      ...
      (define! `(core/defnz ~'hyp [~'a :f64 ~'b :f64 :ret :f64]
                  {:zig/file ~(.getPath path) :c/link ["m"]}))
