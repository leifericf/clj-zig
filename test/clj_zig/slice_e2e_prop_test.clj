(ns clj-zig.slice-e2e-prop-test
...
(deftest fixed-array-rejects-the-wrong-length
  (is (= :clj-zig/array-length (code-from #(f/array4-sum (int-array [1 2 3])))))
  (is (= :clj-zig/array-length (code-from #(f/array4-sum (int-array [1 2 3 4 5]))))))

(deftest single-item-pointer-rejects-a-multi-element-array
  (is (= :clj-zig/pointer-arity (code-from #(f/deref-or-default (long-array [1 2]))))))
