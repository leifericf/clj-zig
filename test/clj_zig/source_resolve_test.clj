...
(deftest candidate-paths-resolves-relative-to-the-defining-file-first
  (is (= ["/home/me/src/app/dot.zig" "dot.zig"]
         (source-file/candidate-paths "/home/me/src/app/core.clj" "dot.zig"))))

(deftest candidate-paths-without-a-defining-file-tries-only-the-cwd
  (testing "nil defining file"
    (is (= ["dot.zig"] (source-file/candidate-paths nil "dot.zig"))))
  (testing "REPL sentinel"
    (is (= ["dot.zig"] (source-file/candidate-paths "NO_SOURCE_PATH" "dot.zig")))))

(deftest candidate-paths-uses-an-absolute-path-as-is
  (is (= ["/opt/zig/dot.zig"]
         (source-file/candidate-paths "/home/me/src/app/core.clj" "/opt/zig/dot.zig"))))
...
(deftest resolve-and-read-reads-a-file-next-to-the-defining-file
  (let [dir   (str (java.nio.file.Files/createTempDirectory
    ...
    (let [{:keys [text path]} (source-file/resolve-and-read (.getPath defn) "body.zig")]
      (is (= "pub fn f() void {}\n" text))
      (is (= (.getPath zig) path)))))

(deftest resolve-and-read-throws-a-structured-diagnostic-when-missing
  (let [ex (try (source-file/resolve-and-read "/nowhere/core.clj" "does-not-exist.zig")
