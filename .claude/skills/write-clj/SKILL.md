---
name: write-clj
description: Recipe for writing Zigar's Clojure code: the pure core (signature, type, spec, source, cache) and the imperative shell (compile, ffm, inspect). Invoked when writing Zigar's Clojure code.
user-invocable: false
---

# write-clj

Write Clojure for Zigar. The standard is
...
1. **Pure core**: `zigar.signature`, `zigar.type`, `zigar.spec`,
   `zigar.source`, `zigar.cache` (hashing only). Data in, data out; no
...
2. **Shell**: `zigar.compile` (invoke `zig`), `zigar.ffm` (load the
   library, bind the symbol via JDK FFM/Panama), `zigar.inspect`
   (render diagnostics and inspection helpers), and `zigar.core` (the
...
- Tests in `test/zigar/<area>_test.clj`. TDD: a failing test against
