...
(defn normalize
  ...
  (cond
    ...
    :else           (fail form :zigar/unknown-type
...
(defn- normalize-scalar [kw]
  (if (contains? scalars kw)
    {:kind :scalar :name kw}
    (fail kw :zigar/unknown-scalar (str "Unknown scalar type " kw ".") {})))
...
(defn- normalize-compound [v]
  (if-let [head (first v)]
    (case head
      ...
      (fail v :zigar/malformed-compound
            (str "Unknown compound type head " (pr-str head) ".") {}))
    (fail v :zigar/malformed-compound "A compound type vector cannot be empty." {})))
  ...
  (let [shape (str (name kind) " takes an optional :const and one element type.")]
    (case (count v)
      ...
      3 (if (= :const (nth v 1))
          ...
          (fail v :zigar/malformed-compound shape {}))
      (fail v :zigar/malformed-compound shape {}))))
