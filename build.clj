(ns build
  ...
  Releases are dated, not numbered. The version is today's date as
  `YYYY.MM.DD` (a Maven-orderable form), and the matching git tag is
  `YYYY-MM-DD`. Set CLJ_ZIG_VERSION to override, for a second release in one
  day or to match an existing tag. The Clojars coordinate is
  `com.leifericf/clj-zig`, the verified group for the leifericf.com domain;
  the git-dep coordinate `io.github.leifericf/clj-zig` keeps working for
...
(def git-tag (str/replace version "." "-"))
